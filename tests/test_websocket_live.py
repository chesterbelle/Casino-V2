...
async def test_websocket_connection():
    ...
    table = TableCCXTPro(exchange_id=EXCHANGE, symbols=SYMBOLS, timeframe=TIMEFRAME, testnet=TESTNET)
    ...
    try:
        ...
        while (datetime.now() - start_time).seconds < 60:
            ...
            logger.info(
                f"📊 Estado {elapsed}s: Conectado={websocket_info.get('connected', False)}, "
...
async def test_multi_symbol_concurrent():
    ...
    table = TableCCXTPro(exchange_id="binance", symbols=SYMBOLS, timeframe="1m", testnet=True)
