...
PY
  ...
  if [ -n "$TIMESTAMPS" ]; then
    ...
    echo "[ronda1] Período detectado: $START_DATE → $END_DATE"
  else
    echo "[ronda1] No se encontraron timestamps en el log, usando período por defecto"
    # Usar últimas 2 horas como fallback
    START_DATE=$(date -d '2 hours ago' '+%Y-%m-%d %H:%M:%S')
    END_DATE=$(date '+%Y-%m-%d %H:%M:%S')
  fi
else
  echo "[ronda1] No se encontró demo log, usando período por defecto"
  START_DATE=$(date -d '2 hours ago' '+%Y-%m-%d %H:%M:%S')
  END_DATE=$(date '+%Y-%m-%d %H:%M:%S')
fi
...
if [ -n "$DEMO_LOG" ] && [ -f "$DEMO_LOG" ] && [ -f "${HIST_DATA}" ]; then
  echo "[ronda1] Filtrando CSV para coincidir con velas procesadas en demo"
...
PY
  ...
  echo "[ronda1] Balance inicial detectado: $INITIAL_BALANCE"
else
  INITIAL_BALANCE=10000
  echo "[ronda1] Usando balance por defecto: $INITIAL_BALANCE"
fi

# 4) Ejecutar backtest con los datos descargados y el mismo balance inicial
echo "[ronda1] Ejecutando backtest con ${HIST_DATA} (balance inicial: $INITIAL_BALANCE)"
...
if [ -n "$DEMO_LOG" ] && [ -n "$BACKTEST_LOG" ]; then
  echo "[ronda1] Demo log: $DEMO_LOG"
  echo "[ronda1] Backtest log: $BACKTEST_LOG"
  python tests/validation/compare_results.py --testing "$DEMO_LOG" --backtest "$BACKTEST_LOG" --output ${COMPARISON_LOG} || echo "[ronda1] compare script devolvió error"
else
  echo "[ronda1] No se encontraron logs para comparar (Demo: ${DEMO_LOG:-None}, Backtest: ${BACKTEST_LOG:-None})"
fi
