...
if [ -z "$DEMO_LOG" ]; then
  echo "[ronda2] No se encontró demo log"
else
...
PY
  ...
  if [ -n "$TIMESTAMPS" ]; then
    ...
    echo "[ronda2] No se encontraron timestamps, usando período por defecto"
...
echo "[ronda2] Descargando datos históricos → ${HIST_DATA}"
python tests/validation/download_historical_data.py --exchange binance --symbol LTC/USDT --interval 1m --start "$START_DATE" --end "$END_DATE" --output ${HIST_DATA} || echo "[ronda2] download failed"
...
PY
  ...
  echo "[ronda2] Balance inicial: $INITIAL_BALANCE"
else
  INITIAL_BALANCE=10000
fi

# 4) Backtest
echo "[ronda2] Ejecutando backtest (balance: $INITIAL_BALANCE)"
...
python tests/validation/compare_results.py --demo-log ${DEMO_LOG:-} --backtest-log ${BACKTEST_LOGS_DIR}/backtest_*.json --out ${COMPARISON_LOG} || echo "[ronda2] compare failed"
