#!/usr/bin/env python3
"""
Test completo del conector Bybit: Validación end-to-end
...
4. Enviar la orden al exchange con TP/SL NATIVO de Bybit
...
**VENTAJA DE BYBIT:** TP/SL nativo en una sola orden (no necesita OCO Monitor)

Uso:
    python tests/test_bybit_complete.py --testnet
"""
...
class BotToExchangeFlowTest:
    """Test del flujo completo Bot → Exchange → Bot con Bybit."""

    def __init__(self, adapter: CCXTAdapter, symbol: str = "BTC/USD:USD", amount: Optional[float] = None):
        """
        Inicializa el test.
...
async def run_test(testnet: bool = True, symbol: str = "BTC/USD:USD", amount: Optional[float] = None) -> bool:
    """
    Ejecuta el test para Bybit.

    Args:
        testnet: Si usar testnet o live
        symbol: Símbolo a testear
        amount: Cantidad (auto-calculada si no se especifica)

    Returns:
        True si el test pasó, False si falló
    """
    logger.info(f"🚀 Iniciando test para: Bybit")
    logger.info(f"🌐 Modo: {'TESTNET' if testnet else 'LIVE'}")
    logger.info(f"📊 Símbolo: {symbol}")

    # Crear conector base de Bybit
    base_connector = BybitConnector(
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Test del flujo Bot → Exchange → Bot con Bybit",
    ...
    # Ejecutar test
    success = asyncio.run(run_test(testnet, args.symbol, args.amount))
