...
def test_memory_csv_sync():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        memory = GeminiMemory(csv_path=csv_path, state_path=state_path, memory_window=100, autosave_interval=10)
...
def test_table_backtest_trade_id_fallback():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        assert result.get("trade_id").startswith(
            "backtest_"
        ), f"trade_id should start with 'backtest_', got {result.get('trade_id')}"
        ...
        assert (
            result2.get("trade_id") == "custom_trade_123"
        ), f"trade_id should be 'custom_trade_123', got {result2.get('trade_id')}"
