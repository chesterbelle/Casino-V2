...
from sensors.mean_reversion import StochasticReversion, BollingerSqueeze
from sensors.momentum_trend_following import Supertrend, ADXFilter
...
def test_stochastic_reversion():
    ...
    for candle in downtrend:
        ...
        if signal:
            ...
            print(f"   ✅ Señal detectada: {signal['side']} - "
...
def test_bollinger_squeeze():
    ...
    for i, candle in enumerate(consolidation):
        ...
        if signal:
            signals.append(signal)
            print(f"   ✅ Breakout detectado: {signal['side']} - "
