...
from .events import Event, SignalEvent, EventType
...
class AggregatedSignalEvent(Event):
    ...
    def __init__(self, symbol: str, candle_timestamp: float, long_votes: int, 
                 short_votes: int, total_sensors: int, side: str, confidence: float):
...
class SignalAggregatorV3:
    ...
    async def _process_signals(self, candle_ts: float):
        ...
        # Log voting results
        logger.info(f"📊 Voting Results: {long_votes} LONG, {short_votes} SHORT → {side} (confidence: {confidence:.2%})")
