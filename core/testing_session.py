"""Runtime loop for testing sessions (Kraken Demo / paper trading)."""
...
TESTING_SLEEP_SECONDS = float(getattr(config, "LIVE_SLEEP_SECONDS", 1.0))
RESULT_LOGGER = logging.getLogger("TestingSession")
...
def _print_testing_summary(stats: Dict) -> None:
...
async def run_testing_session(
    ...
    """Ejecuta el loop testing reutilizando BrokerInterface (exchange demo)."""
    logging.getLogger().setLevel(getattr(logging, config.LOG_LEVEL, logging.INFO))
    exchange = getattr(config, "EXCHANGE", "SIMULATION").upper()
    RESULT_LOGGER.info("Iniciando sesión testing (exchange=%s)", exchange)

    if player_module is None:
        raise ValueError("Se requiere un player_module para ejecutar sesiones de testing.")
    ...
    # En modo testing (demo), intentar obtener balance real del exchange
    real_balance = None
    if hasattr(table, "exchange") and table.exchange:
        try:
            ...
            # CRÍTICO: En modo testing nunca usar balance simulado
            # Si no se puede obtener balance real, DETENER el sistema
            raise RuntimeError(
                f"❌ MODO TESTING: No se pudo obtener balance demo del exchange.\n"
                f"Error: {error_msg}\n"
    ...
    # En modo testing, requerir balance real/demonstrativo
    if real_balance is not None and real_balance > 0:
        ...
        # En modo testing tampoco podemos continuar sin balance demo válido
        ...
        raise RuntimeError(
            "❌ MODO TESTING requiere balance del exchange demo. "
            "Verifique que su cuenta demo tenga fondos disponibles. "
            f"Balance obtenido: {real_balance}. "
            "Para Kraken Demo, asegúrese de que la cuenta demo esté financiada."
        )

    RESULT_LOGGER.info(
        "Mesa testing lista | symbol=%s | interval=%s | balance=%.4f | equity=%.4f | source=%s",
        symbol,
        interval,
        initial_balance,
        initial_equity,
    ...
    RESULT_LOGGER.info("✅ Sesión testing inicializada correctamente")
    ...
    try:
        while True:
            ...
            try:
                ...
                RESULT_LOGGER.exception("Fallo al enrutar orden testing: %s", exc)
                await asyncio.sleep(TESTING_SLEEP_SECONDS)
                if limit_reached():
                    register_limit_exit()
                    break
            ...
            RESULT_LOGGER.info(
                "🎰 TESTING TRADE | %s %s | action=%s | result=%s | Unidad=%su(%.2f %s)",
                order.get("symbol"),
                order.get("side"),
                action_label,
                result.get("result"),
        ...
        RESULT_LOGGER.info("Sesión testing finalizada por el usuario.")
    finally:
